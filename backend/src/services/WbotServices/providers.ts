...
function getCpfCnpj(msg: proto.IWebMessageInfo) {
  let cpfcnpj
...
function getValidCpfCnpj(numberCPFCNPJ) {
...
export const provider = async (ticket: Ticket, msg: proto.IWebMessageInfo, companyId: number, contact: Contact, wbot: WASocket) => {
  ...
  if (filaescolhida != "2ª Via de Boleto" && filaescolhida != "2 Via de Boleto"
    && filaescolhida != "Religue de Confiança" && filaescolhida != "Liberação em Confiança") {
    return;
  }
  let cpfcnpj = getCpfCnpj(msg);

  var settings = await Setting.findAll({
    where: {
      companyId,
      key: {
        [Op.or]: ["asaas", "tokenixc", "ipixc", "ipmkauth", "clientidmkauth", "clientsecretmkauth"]
      }
    }
  });

  const asaastoken = settings.find(setting => setting.key === "asaas");
  const ixcapikey = settings.find(setting => setting.key === "tokenixc");
  const urlixcdb = settings.find(setting => setting.key === "ipixc");
  const ipmkauth = settings.find(setting => setting.key === "ipmkauth");
  const clientidmkauth = settings.find(setting => setting.key === "clientidmkauth");
  const clientesecretmkauth = settings.find(setting => setting.key === "clientsecretmkauth");
  ...
  let numberCPFCNPJ = getValidCpfCnpj(cpfcnpj);
  if (numberCPFCNPJ === null) {
                ...
                try {
                  ...
                  (async () => {
                    ...
                    axios({
                      ...
                      .then(async function (response) {
                        fs.writeFileSync(nomePDF, response.data);
          ...
          axios.request(optionsListpaymentOVERDUE as any).then(async function (response) {
            ...
            if (totalCount_overdue === 0) {
              ...
              axios.request(optionsPENDING as any).then(async function (response) {
                ...
                value_pending_corrigida = value_pending.toLocaleString('pt-br', {
                  style: 'currency',
                  currency: 'BRL'
                });
              ...
              value_overdue_corrigida = value_overdue.toLocaleString('pt-br', {
                style: 'currency',
                currency: 'BRL'
              });
              const body = {
  ...
  } else if (filaescolhida === "Religue de Confiança" || filaescolhida === "Liberação em Confiança") {
