...
const TicketsListCustom = (props) => {
    ...
    useEffect(() => {
       ...
       // const filteredTickets = tickets.filter((t) => queueIds.indexOf(t.queueId) > -1);
        ...
        if (profile === "user" && allTicket === "disabled") {
            dispatch({ type: "LOAD_TICKETS", payload: ticketWithQueue });
        } else {
            dispatch({type: "LOAD_TICKETS", payload: tickets});
        }
    }, [tickets, status, searchParam, queues, profile, allTicket]);
    ...
    useEffect(() => {
        ...
        const nonGroup = ticketsList.filter(ticket => !ticket.isGroup).length;
        if (typeof updateCount === "function") {
            updateCount(nonGroup);
        }

        //console.log("nonGroup " + nonGroup);
        // console.log("group " + (ticketsList.length - nonGroup));

        if (typeof updateGroupCount === "function") {
            updateGroupCount(ticketsList.length - nonGroup);
        }
